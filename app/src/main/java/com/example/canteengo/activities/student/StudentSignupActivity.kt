...
import com.example.canteengo.databinding.ActivityStudentSignupBinding
...
class StudentSignupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.etLibraryCard.addTextChangedListener(SimpleTextWatcher { binding.tilLibraryCard.error = null })
        binding.etDepartment.addTextChangedListener(SimpleTextWatcher { binding.tilDepartment.error = null })
        binding.etDivision.addTextChangedListener(SimpleTextWatcher { binding.tilDivision.error = null })
    ...
    private fun signUp() {
        ...
        val dept = binding.etDepartment.text?.toString()?.trim().orEmpty()
        ...
        if (!validate(name, libraryCard, dept, division, mobile, email, pass)) return
        ...
        lifecycleScope.launch {
            try {
                ...
                // Requirement: after signup, go back to login.
